    ...
    )
{
    ...
    if (EFI_ERROR(Status)) {
        DEBUG((DEBUG_ERROR, "SdHost: SdHostSetClockFrequency(): Failed to query core clock\n"));
        return Status;
    }
    ...
    )
{
    ...
    DEBUG((
        (SdCmd & (SDHOST_CMD_WRITE_CMD | SDHOST_CMD_READ_CMD))?DEBUG_ERROR:DEBUG_MMCHOST_SD,
        ...
        while (PollCount < CMD_MAX_POLL_COUNT) {
            ...
            if (CmdReg & SDHOST_CMD_FAIL_FLAG) {
				...
				if((MmcCmd & MMC_CMD_NO_CRC_RESPONSE)
					&& ((HSTS == SDHOST_HSTS_CRC7_ERROR) || (HSTS == SDHOST_HSTS_CRC16_ERROR))) {
					Status = EFI_SUCCESS;
					MmioWrite32(SDHOST_HSTS, SDHOST_HSTS_CLEAR);
				}
				else {
    ...
    )
{
	...
	// We are EMMC and power up, set RCA to 0xCCCC
	if ((mLastExecutedMmcCmd == MMC_CMD1) && 
		(Buffer[0] & BIT31)){
        mRca = 0xCCCC;
		mEmmcCard = TRUE;
    }
    ...
    )
{
    ...
    case MmcHwInitializationState:
    {
		...
		MmioWrite32(SDHOST_HBCT, 512); // Set default block size, it will be changed by CM16
    ...
    case MmcTransferState:
    {
        ...
        if (!mIsSdBusSwitched4BitMode && (!mEmmcCard)) {
