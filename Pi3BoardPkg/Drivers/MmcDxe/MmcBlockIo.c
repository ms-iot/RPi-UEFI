...
#define EMMC_IDENT_RETRY_COUNT 		5
#define EMMC_IDENT_WAIT_TIME		10000
  ...
  )
{
    ...
    EFI_MMC_HOST_PROTOCOL   *MmcHost;
	UINT32 retry = EMMC_IDENT_RETRY_COUNT;	
	...
	while(retry) {
		...
		DEBUG ((EFI_D_ERROR, "IsEmmcDevice() : MMC_CMD1, Status=%r.\n", Status));
		if (Status == EFI_SUCCESS) {
		  Status = MmcHost->ReceiveResponse (MmcHost, MMC_RESPONSE_TYPE_OCR, (UINT32 *)&OcrResponse);
		  if (EFI_ERROR (Status)) {
			DEBUG ((EFI_D_ERROR, "IsEmmcDevice() : Failed to receive OCR, Status=%r.\n", Status));
		  }
		
		  if (!OcrResponse.Ocr.PowerUp) {
			DEBUG ((EFI_D_ERROR, "IsEmmcDevice(MMC_CMD1): Card power down %d retry.\n", retry));
		  }
		  else {
			  ...
			  DEBUG ((EFI_D_ERROR, "IsEmmcDevice(MMC_CMD1): OCR %x.\n", OcrResponse.Raw));
		...
		gBS->Stall(EMMC_IDENT_WAIT_TIME);
		retry--;
	}
...
)
{
  ...
  MmcHostInstance->CardInfo.RCA = 0xCCCC; // TODO: might need a more sophisticated way of doing this
    ...
    )
{
	...
	if(IsEmmcDevice(MmcHostInstance))
	{
		return EmmcIdentificationMode (MmcHostInstance);
	}
    ...
    )
{
	...
	if ((MmcHostInstance->CardInfo.CardType == SD_CARD) 
		 || (MmcHostInstance->CardInfo.CardType == SD_CARD_2_SDHC)){
