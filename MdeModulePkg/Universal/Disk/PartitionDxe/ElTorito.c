  ...
  )
{
  ...
  // Ensure 2048 is a multiple of block size
  if (((2048 % Media->BlockSize) != 0) && (Media->BlockSize > 2048)) {
  ...
  while (TRUE) {
    ...
    for (Index = 1, BootEntry = 1; Index < MaxIndex; Index += 1) {
      ...
      Status = PartitionInstallChildHandle (
                ...
                Catalog->Boot.Lba * (2048 / Media->BlockSize),
                (Catalog->Boot.Lba + CdDev.PartitionSize - 1) * (2048 / Media->BlockSize),
                SubBlockSize,
                FALSE
                );
      if (!EFI_ERROR (Status)) {
