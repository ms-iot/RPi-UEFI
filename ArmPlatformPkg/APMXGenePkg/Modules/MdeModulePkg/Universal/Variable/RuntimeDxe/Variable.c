  ...
  )
{
  ...
  volatile EFI_PHYSICAL_ADDRESS        DataPtr;
  EFI_STATUS                  Status;
  ...
  )
{
  ...
  //
  Status = GetFvbInfoByAddress (NvStorageBase, NULL, &FvbProtocol);
  if (EFI_ERROR (Status)) {
    return EFI_VOLUME_CORRUPTED;
  }

  if (FvbProtocol->GetMappedAddress) {
     Status = FvbProtocol->GetMappedAddress(FvbProtocol, &NvStorageBase);
     if (EFI_ERROR (Status)) {
       return EFI_VOLUME_CORRUPTED;
     }
  }
  ...
  FvHeader = (EFI_FIRMWARE_VOLUME_HEADER *) NvStorageBase;
  ...
  )
{
...
#else
  DEBUG ((EFI_D_ERROR, "VariableCommonInitialize 12\n"));
  ...
  // Get the proper FVB protocol.
  //
  Status = GetFvbInfoByAddress (TempVariableStoreHeader, NULL, &FvbProtocol);
  if (EFI_ERROR (Status)) {
    Status = EFI_VOLUME_CORRUPTED;
  DEBUG ((EFI_D_ERROR, "VariableCommonInitialize 13\n"));
    goto Done;
  }

  if (FvbProtocol->GetMappedAddress) {
    DEBUG ((EFI_D_ERROR, "VariableCommonInitialize 14\n"));
    Status = FvbProtocol->GetMappedAddress(FvbProtocol, &TempVariableStoreHeader);
    if (EFI_ERROR (Status)) {
      Status = EFI_VOLUME_CORRUPTED;
      goto Done;
    }
  }
  DEBUG ((EFI_D_ERROR, "VariableCommonInitialize 15\n"));
  ...
  )
{
  ...
  for (Index = 0; Index < HandleCount; Index += 1, Status = EFI_NOT_FOUND, Fvb = NULL) {
    ...
    DEBUG ((EFI_D_BLKIO, "FvbBaseAddress: 0x%p\n", FvbBaseAddress));
