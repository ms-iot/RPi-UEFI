...
size_t iob_cnt;
unsigned char iobdb[NUM_IOB_SLOTS][IOB_SLOT_SZ];

unsigned poll_verbosity;
		   ...
		   struct io_buffer *iobuf )
{
	...
	if ( qp->send.fill >= qp->send.num_wqes ) {
		ERROR("IBDEV %p QPN %lx send queue full\n",
		       ibdev, qp->qpn );
		return -ENOBUFS;
	}
	...
	if ( ( rc = ibdev->op->post_send ( ibdev, qp, av, iobuf ) ) != 0 ) {
		ERROR("IBDEV %p QPN %lx could not post send WQE: "
		       "%s\n", ibdev, qp->qpn, strerror ( rc ) );
		return rc;
	}
...
void ib_refill_recv ( struct ib_device *ibdev, struct ib_queue_pair *qp ) {
	...
	assert( qp->recv.num_wqes <= HERMON_ETH_NUM_RECV_WQES);
	assert( MAX_PACKET_SZ >= IB_MAX_PAYLOAD_SIZE);

	/* Keep filling while unfilled entries remain */
	while ( qp->recv.fill < qp->recv.num_wqes ) {

		/* Allocate I/O buffer */
		iobuf = alloc_iob (NULL, IB_MAX_PAYLOAD_SIZE );
		if ( ! iobuf ) {
...
int ib_link_rc ( struct ib_device *ibdev ) {
	...
	case IB_PORT_STATE_DOWN:	return -EINVAL;
	case IB_PORT_STATE_INIT:	return -EINVAL;
	case IB_PORT_STATE_ARMED:	return -EINVAL;
...
int ib_count_ports ( struct ib_device *ibdev ) {
	int num_ports = 0;
	num_ports = ibdev->op->num_phy_ports ( ibdev );
...
void ib_poll_eq ( struct ib_device *ibdev ) {
	...
	if (unlikely(!hermon->open_count)) {
		DBGC( ibdev, "IBDEV %p hermon NIC is closed. Cowardly refuse to execute ib_poll_eq\n",
		      ibdev);
		return;
	}
	...
	list_for_each_entry ( cq, &ibdev->cqs, list ) {
		if (unlikely(!hermon->open_count)) {
			DBGC( ibdev, "IBDEV %p hermon NIC is closed. Cowardly refuse to continue ib_poll_eq\n",
			      ibdev);
			return;
		}
