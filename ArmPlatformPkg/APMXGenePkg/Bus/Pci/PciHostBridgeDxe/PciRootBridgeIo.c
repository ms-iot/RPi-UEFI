  ...
  )
{
  ...
  if (OperationType == IoOperation) {
    ...
    if (PciRbAddr->Bus == XGENE_PCIE_FIRST_BUS && PciRbAddr->Device != 0) {
      return EFI_INVALID_PARAMETER;
    }

    /* The other side of the RC has only one device as well */
    if (PciRbAddr->Bus == (XGENE_PCIE_FIRST_BUS + 1 ) && PciRbAddr->Device != 0) {
      return EFI_INVALID_PARAMETER;
    }
...
//  PCIE_DEBUG("RootBridgeIoPciRW Bus:%d Dev:%d Func:%d\n", PciRbAddr->Bus, PciRbAddr->Device, PciRbAddr->Function);
//  PCIE_DEBUG("RootBridgeIoPciRW PciRbAddr->ExtendedRegister:0x%llx\n", PciRbAddr->ExtendedRegister);
//  PCIE_DEBUG("RootBridgeIoPciRW PciRbAddr->Register:0x%llx\n", PciRbAddr->Register);
//  PCIE_DEBUG("RootBridgeIoPciRW PcieRegAddr:0x%llx\n", PcieRegAddr);
//  PCIE_DEBUG("RootBridgeIoPciRW Count:0x%llx\n", Count);
//  PCIE_DEBUG("RootBridgeIoPciRW Width:0x%llx\n", Width);
  ...
  for (Uint8Buffer = Buffer; Count > 0; PcieRegAddr += InStride, Uint8Buffer += OutStride, Count--) {
    if (Write) {
      switch (OperationWidth) {
        case EfiPciWidthUint8:
          XGenePcieWriteConfig8 (PrivateData, PcieRegAddr, *Uint8Buffer);
          break;
        case EfiPciWidthUint16:
          XGenePcieWriteConfig16 (PrivateData, PcieRegAddr, *((UINT16 *)Uint8Buffer));
          break;
        case EfiPciWidthUint32:
          XGenePcieWriteConfig32 (PrivateData, PcieRegAddr, *((UINT32 *)Uint8Buffer));
          break;
        default:
          //
          // The RootBridgeIoCheckParameter call above will ensure that this
      ...
      switch (OperationWidth) {
        case EfiPciWidthUint8:
          XGenePcieReadConfig8 (PrivateData, PcieRegAddr, Uint8Buffer);
          break;
        case EfiPciWidthUint16:
          XGenePcieReadConfig16 (PrivateData, PcieRegAddr, (UINT16 *)Uint8Buffer);
          break;
        case EfiPciWidthUint32:
          XGenePcieReadConfig32 (PrivateData, PcieRegAddr, (UINT32 *)Uint8Buffer);
          break;
        default:
          //
          // The RootBridgeIoCheckParameter call above will ensure that this
